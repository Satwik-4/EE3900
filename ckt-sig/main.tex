...
\author{ Satwik Sajja %<-this  stops a space
...
\end{figure}
	...
	\begin{lstlisting}
wget https://raw.githubusercontent.com/Satwik-4/EE3900/master/ckt-sig/codes/2_7.py
\end{lstlisting}
	\begin{figure}[h!]
	    \centering
...
\begin{lstlisting}
wget https://raw.githubusercontent.com/Satwik-4/EE3900/master/ckt-sig/codes/2_8.cir
\end{lstlisting}
	\begin{figure}[h!]
	    \centering
...
The following code plots Fig. \ref{fig:vct2}
	\begin{lstlisting}
wget https://raw.githubusercontent.com/Satwik-4/EE3900/master/ckt-sig/codes/3_4.py
\end{lstlisting}
	\begin{figure}[h!]
	    \centering
...
\begin{lstlisting}
wget https://raw.githubusercontent.com/Satwik-4/EE3900/master/ckt-sig/codes/3_5.cir
\end{lstlisting}
	\begin{figure}[h!]
	    \centering
...
\begin{lstlisting}
wget https://raw.githubusercontent.com/Satwik-4/EE3900/master/ckt-sig/codes/4_3.py
\end{lstlisting}
	\begin{figure}[h!]
	    \centering
	    ...
	    \label{fig:transfer}
	\end{figure}
$H(s)$ looks like a low-pass filter
	\item Using trapezoidal rule for integration, formulate the difference equation
		by considering 
	\begin{align}
		y(n) = y(t)\vert_{t=n}
	\end{align}\\\solution
	Using \eqref{eq:vct-de} and applying the trapezoidal rule:
	\begin{align}
	\int_a^bf(x)dx &= \frac{1}{2}\brak{b-a}\brak{f(b)+f(a)}\\
	C\dot{V_c}+\frac{3V_c}{2} &= \frac{V_2}{2}
	\end{align}
	Applying the limits from n to n+1
	\begin{align}
	    	C\int_{v_c(n1)}^{v_c(n+1)} dV_c + \int_{n}^{n+1}\frac{3V_c}{2}dt&= \int_{n}^{n+1}u(t) dt\\
	    	\implies C\brak{v_c(n+1)-v_c(n)}&+\nonumber\\
	    	\frac{3}{4}\brak{v_c(n+1)+v_c(n)} =\frac{u(n+1)+u(n)}{2}&\\
	    	v_c(n+1)\brak{C+0.75}=v_c(n)\brak{C-0.75}&+\nonumber\\
	    	\frac{u(n+1)+u(n)}{2}&
	\end{align}
	\item Find $H(z)$.
	\begin{align}
	    H(z) &= \frac{V_c(z)}{V_2(z)}\label{eq:hz-def}
	\end{align}
	where
	\begin{align}
	    v_2(n) &= 2u(n)\\
